...
class TalkScriberTTSClient:
    def __init__(
        ...
        if not DEPENDENCIES_AVAILABLE:
            raise ImportError(
                "Required dependencies (pyaudio, websocket-client, loguru) are not installed. Please install them with: pip install pyaudio websocket-client loguru"
            )
    ...
    def set_callbacks(
        self, on_open=None, on_message=None, on_error=None, on_close=None
    ):
    ...
    def connect(self):
        ...
        try:
            ...
            self.websocket_client = websocket.WebSocketApp(
                ...
                on_close=lambda ws, status_code, msg: self.on_close(
                    ws, status_code, msg
                ),
            )
    ...
    def send_speak_request(self, text, speaker_name=None):
        ...
        logger.info(
            f"Sending speak request for text: '{text[:50]}{'...' if len(text) > 50 else ''}'"
        )
    ...
    def init_audio(self):
        ...
        try:
            ...
            logger.info(
                f"Audio initialized: {SAMPLE_RATE}Hz, {CHANNELS} channel(s), {BITS_PER_SAMPLE}-bit"
            )
            return True
        except Exception as e:
    ...
    def save_audio_to_file(self):
        ...
        try:
            ...
            logger.info(
                f"Audio saved to: {self.save_audio_path} ({len(combined_audio):,} bytes, WAV format)"
            )
            return True
    ...
    def handle_audio_chunk(self, chunk):
        ...
        # Calculate TTFT (Time To First Token) for the first chunk
        if self.chunks_received == 1 and self.request_start_time is not None:
            self.first_token_time = time.time()
            self.ttft_seconds = self.first_token_time - self.request_start_time
            logger.info(
                f"⚡ TTFT: {self.ttft_seconds * 1000:.2f}ms (Time to First Token)"
            )

        logger.info(
            f"Received audio chunk #{self.chunks_received}: {len(chunk)} bytes (total: {self.total_bytes:,} bytes)"
        )
        ...
        if self.enable_playback:
            ...
            if buffer_size >= BUFFER_SIZE_CHUNKS and not self.is_playing:
                logger.info(
                    f"Buffer filled ({buffer_size} chunks), starting audio playback..."
                )
                self.start_audio_playback()
            elif buffer_size == 1 and not self.is_playing:
                logger.info(
                    f"First chunk buffered, waiting for {BUFFER_SIZE_CHUNKS} chunks before starting playback..."
                )

            # Log buffer status periodically
            if self.chunks_received % 10 == 0:
                logger.debug(
                    f"Audio buffer status: {buffer_size} chunks buffered, playing: {self.is_playing}"
                )
    ...
    def handle_json_message(self, data):
        ...
        if message_type == "authenticated":
            ...
            if self.enable_playback:
                logger.info(
                    "Audio generation finished - letting buffered audio finish playing..."
                )
                ...
                if self.is_playing:
                    ...
                    if buffer_size > 0:
                        logger.info(
                            f"Waiting for {buffer_size} remaining chunks to play..."
                        )
            ...
            if self.send_speak_request(self.text):
                ...
                if self.enable_playback and self.is_playing:
                    ...
                    if len(self.audio_buffer) > 0:
                        logger.warning(
                            f"Audio buffer still has {len(self.audio_buffer)} chunks after waiting"
                        )
                    else:
    ...
    def get_audio_info(self):
        ...
        with self.storage_lock:
            ...
            return {
                ...
                "ttft_ms": self.ttft_seconds * 1000
                if self.ttft_seconds is not None
                else None,
            }

    def get_ttft_metrics(self):
        """Get TTFT (Time To First Token) metrics"""
        return {
